...
public:
 ...
 bool request_for_driver(int joint,uint8_t command,uint8_t data,double value,double pos)
  ...
  void analysisCB(const sensor_msgs::JointState::ConstPtr& arm)
  {
    ...
    feedback_.joint_names.resize(5);
    feedback_.actual.positions.resize(5);
    feedback_.actual.velocities.resize(5);
	...
	case 3:
          ...
          feedback_.actual.positions[i] = arm->position[i+1];
          ros::param::get("/speedklb4",feedback_.actual.velocities[i+1]);
          break;
	case 4:
          feedback_.joint_names[i] = "klb5";
          feedback_.actual.positions[i] = arm->position[i+1];
          ros::param::get("/speedklb5",feedback_.actual.velocities[i+1]);
        break;
       }
    }
    ...
    {
      /*ROS_INFO("Vzorky_real=%d",vzorky_real);
	...
	/*for (int i = 0; i < 6; i++)// to_do dorobit kontrolu case pre aktualny cas a realny cas simulacie
	{
	  switch (i)
	  {
	   ...
	   case 3: 
	 //   {
	//      request_for_driver(i,comm_message,comm_motory_speed,goal_.points[vzorky_real].velocities[i]);
	 //     break;
	 //   }
	   case 4: 
	    {
	...
	for (int i = 0; i < 5; i++)// to_do dorobit kontrolu case pre aktualny cas a realny cas simulacie
	{
	  switch (i)
	  {
	    case 0: 
	    {
	      request_for_driver(i,comm_message,comm_position,goal_.points[vzorky-2].velocities[i],goal_.points[vzorky-1].positions[i]);
	      break;
	    }
	   case 1: 
	    {
	...
	//      request_for_driver(i,comm_message,comm_motory_speed,goal_.points[vzorky_real].velocities[i]);
	 ...
	 //   }*/
	   case 3: 
	    {
	      request_for_driver(i+1,comm_message,comm_position,goal_.points[vzorky-2].velocities[i],goal_.points[vzorky-1].positions[i]);
	      break;
	    }
	   case 4: 
	    {
	...
	vzorky_real--;
      ...
      ROS_INFO("aktualne trvanie goalu sec:%d nsec %d",aktual.sec, aktual.nsec);

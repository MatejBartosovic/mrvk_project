#include "kv01_driver/common_functions.h"
    ...
    bool CommonFunctions::init_driver() {
      for (int joint = 0; joint < 6; joint++) {
        odbrzdene[joint] = false;
        std::string joint_string = CommonFunctions::to_string(joint);
    ...
    uint8_t CommonFunctions::Calculate_CRC(uint8_t *buffer, unsigned int num) {
    ...
    unsigned short CommonFunctions::Calculate_HL(uint8_t H, uint8_t L) {
      unsigned short a;
      a = H << 8;
      a = L | a;
    ...
    std::string CommonFunctions::to_string(int i) {
      std::stringstream ss;
      ss << i;
      return ss.str();
    }

    bool CommonFunctions::check_response(uint8_t *buffer) {
    ...
    uint8_t *CommonFunctions::ArmCreateD0(uint8_t data, int joint, double value) {
      ...
      switch (data) {
        ...
        default: {
          ...
          ROS_ERROR("Error: CommonFunctions::ArmCreateD0: Neznamy variant dat prikazu D0 \n");
    ...
    bool CommonFunctions::ArmReceiveD0(uint8_t *buffer, int joint, uint8_t data) {
      switch (data) {
    ...
    bool CommonFunctions::ArmReceiveD1(uint8_t *buffer, uint8_t data, int joint) {
    ...
    uint8_t *CommonFunctions::ArmCreateD1(uint8_t *message, int joint, uint8_t data) {
      ...
      requestD1.crc = CommonFunctions::Calculate_CRC(message, sizeof(requestD1) - 1);
      return message;
    }

    uint8_t *CommonFunctions::ArmCreateD3(uint8_t *message, int joint) {
      ...
      requestD3.crc = CommonFunctions::Calculate_CRC(message, sizeof(requestD3) - 1);
      return message;
    }

    double CommonFunctions::ArmReceiveD3(uint8_t *buffer, int joint) {
      ...
      if (joint < 5 && joint >= 0) {
        ...
        angle_compute = CommonFunctions::Calculate_HL(p->data1, p->data2);
    ...
    uint8_t *CommonFunctions::ArmCreateD4(uint8_t *message, int joint) {
      ...
      requestD4.crc = CommonFunctions::Calculate_CRC(message, sizeof(requestD4) - 1);
      return message;
    }


    void CommonFunctions::ArmReceiveD4(uint8_t *buffer) {
      ...
      voltage_compute = (CommonFunctions::Calculate_HL(p->volt24H, p->volt24L));
      rob_arm_voltages.volt24V_j = ((double) voltage_compute * machine_data.voltage24Vconst);

      voltage_compute = (CommonFunctions::Calculate_HL(p->volt12H, p->volt12L));
      rob_arm_voltages.volt12V_j = ((double) voltage_compute * machine_data.voltage12Vconst);

      voltage_compute = (CommonFunctions::Calculate_HL(p->volt5H, p->volt5L));
      rob_arm_voltages.volt5V_j = ((double) voltage_compute * machine_data.voltage5Vconst);

      return;
    }

    uint8_t *CommonFunctions::ArmCreateD5(uint8_t *message, int joint) {
      ...
      requestD5.crc = CommonFunctions::Calculate_CRC(message, sizeof(requestD5) - 1);
